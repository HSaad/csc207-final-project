...
//		}
		...
		try{
			...
			while((line = br.readLine()) != null ){
				...
				if (pcat.equals("PRODUCT")) {
					...
					//System.out.println(this.inventory.getDistributionCenters());
	...
	public void changeProductQuantity(Product product, int quantity, DistributionCenter DC){
		//need to change the quantity of product in the specified Distribution center
	...
	public void addDistributionCenerFile(DistributionCenter DC){
		...
		try {
			...
			//sets the quantity of DC products in a file
			//for (Product product : DC.getQuantity().keySet()){
			//	this.changeProductQuantity(product, DC.getQuantity().get(product), DC);
			//}
	...
	public void addProductToFile(Product product){
		...
		try{
			...
			//write a new product to the csv file
			//if statement to stop it from adding the same product 
		//	if (!this.inventory.getProductList().contains(product)){
		//	System.out.println("file Content Size: "+fileContent.size());
		//	System.out.println("intventroy list size: "+this.inventory.getProductList().size() + 1);
			
			
			
	//		if(fileContent.size() < this.inventory.getProductList().size() + 1){
				//fw.append("\n");
			//	System.out.println("DM: " + product.getID());
				fw.append("" + product.getID());
				
				//int firstQuant = product.getQuantity();
				String line = "";
				int quantity =0;
				
			//	int quantDC = Math.round(product.getQuantity()/(cities.length - 1));
